    ...
    if not arguments['--testlauf']:
        while True:
            try:
                ...
                print(time.strftime("%Y-%m-%d %H:%M:%S") + " - Fehler im Suchlauf: " % e)
    else:
        try:
            start_time = time.time()
            ...
            if download_link:
                if self.db.retrieve(video) == 'added':
                    ...
                    if ignorevideo:
                        ...
                        continue
                    common.write_crawljob_file(video, "YouTube/" + channel, download_link, jdownloaderpath + "/folderwatch", "RSScrawler")
                    self.db.store(video, 'added')
...
class SJ():
    ...
    def periodical_task(self):
        if self.filename == "MB_Staffeln" or self.filename == "SJ_Staffeln_Regex":
            feed = feedparser.parse(str(base64.b64decode("aHR0cDovL3Nlcmllbmp1bmtpZXMub3JnL3htbC9mZWVkcy9zdGFmZmVsbi54bWw="), 'utf-8'))
        else:
            feed = feedparser.parse(str(base64.b64decode("aHR0cDovL3Nlcmllbmp1bmtpZXMub3JnL3htbC9mZWVkcy9lcGlzb2Rlbi54bWw="), 'utf-8'))
        ...
        if self.empty_list:
            return
        try:
            reject = self.config.get("rejectlist").replace(",", "|").lower() if len(self.config.get("rejectlist")) > 0 else r"^unmatchable$"
        except TypeError:
                            ...
                            if mm:
                                ...
                                if rsscrawler.get("surround"):
                                    if not re.match(r'.*\.(DTS|DD\+*51|DD\+*71|AC3\.5\.*1)\..*', title):
                                        self.log_debug(title + " - Release ignoriert (kein Mehrkanalton)")
                                        continue
                                title = re.sub(r'\[.*\] ', '', post.title)
                                self.range_checkr(link, title, language_ok)
                        else:
...
class MB():
    ...
    def searchLinks(self, feed):
        if self.empty_list:
            return
        ignore = "|".join([r"\.%s(\.|-)" % p for p in self.config.get("ignore").lower().split(',')]) if self.config.get("ignore") else r"^unmatchable$"
                ...
                if found:
                    ...
                    if rsscrawler.get("surround"):
                        if not re.match(r'.*\.(DTS|DD\+*51|DD\+*71|AC3\.5\.*1)\..*', post.title):
                            self.log_debug(post.title + " - Release ignoriert (kein Mehrkanalton)")
                            continue
                            ...
                            if self.config.get('crawl3d') and ("1080p" in post.title.lower() or "1080i" in post.title.lower()):
                                ...
                                if c3d_type == "hsbs":
                                    if re.match(r'.*\.(H-OU|HOU)\..*', post.title):
                                        self.log_debug("%s - Release ignoriert (Falsches 3D-Format)" % post.title)
                                        continue
                                elif c3d_type == "hou":
                ...
                if self.db.retrieve(key) == 'added' or self.db.retrieve(key) == 'dl':
                    ...
                    if self.config.get('cutoff'):
                        if self.config.get('enforcedl'):
                            ...
                            if common.cutoff(key, '0'):
                                retail = True
                    common.write_crawljob_file( key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler/Remux")
                    self.db.store( key,'dl' if self.config.get('enforcedl') and '.dl.' in key.lower() else 'added')
                    ...
                    if self.config.get('cutoff'):
                        if common.cutoff(key, '2'):
                            retail = True
                    common.write_crawljob_file(key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler/3Dcrawler")
                    self.db.store( key, 'dl' if self.config.get('enforcedl') and '.dl.' in key.lower() else 'added')
                    ...
                    common.write_crawljob_file(key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler")
                    self.db.store( key, 'dl' if self.config.get('enforcedl') and '.dl.' in key.lower() else 'added')
        ...
        for item in imdbchecked:
            ...
            if rsscrawler.get("surround"):
                if not re.match(r'.*\.(DTS|DD\+*51|DD\+*71|AC3\.5\.*1)\..*', download_title):
                    self.log_debug(download_title + " - Release ignoriert (kein Mehrkanalton)")
                    continue
                    ...
                    if self.config.get('crawl3d') and ("1080p" in download_title.lower() or "1080i" in download_title.lower()):
                        ...
                        if c3d_type == "hsbs":
                            if re.match(r'.*\.(H-OU|HOU)\..*', download_title):
                                self.log_debug("%s - Release ignoriert (Falsches 3D-Format)" % download_title)
                                continue
                        elif c3d_type == "hou":
                        ...
                        if self.config.get('cutoff') and '.COMPLETE.' not in key.lower():
                            if self.config.get('enforcedl'):
                                ...
                                if common.cutoff(key, '0'):
                                    retail = True
                    common.write_crawljob_file( key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler" )
                    self.db.store(key, 'notdl' if self.config.get( 'enforcedl') and '.dl.' not in key.lower() else 'added')
    ...
    def periodical_task(self):
        ...
        if self.filename == 'MB_Staffeln':
            if not self.config.get('crawlseasons'):
                return
            self.allInfos = dict( set({key: value for (key, value) in list(self.getPatterns( self.readInput(self.search_list), quality=self.config.get('seasonsquality'), rg='.*', sf=('.complete.')).items())}.items()))
        elif self.filename == 'MB_Regex':
            if not self.config.get('regex'):
                return
            self.allInfos = dict(set({key: value for (key, value) in list(self.getPatterns(self.readInput(self.search_list)).items())}.items()) if self.config.get('regex') else [])
        else:
            if self.filename == 'MB_3D':
                if not self.config.get('crawl3d'):
                    return
            self.allInfos = dict(set({key: value for (key, value) in list(self.getPatterns(self.readInput(self.search_list), quality=self.config.get('quality'), rg='.*', sf=None).items())}.items()))
            ...
            for (key, value, pattern) in self.searchLinks(feedparser.parse(url)):
                ...
                if download_link:
                    ...
                    if self.db.retrieve(key) == 'added' or self.db.retrieve(key) == 'notdl' or self.db.retrieve(key.replace(".COMPLETE", "").replace(".Complete", "")) == 'added':
                        ...
                        common.write_crawljob_file(key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler")
                        self.db.store(key.replace(".COMPLETE", "").replace(".Complete", ""), 'notdl' if self.config.get('enforcedl') and '.dl.' not in key.lower() else 'added')
            ...
            for (key, value, pattern) in self.searchLinks(feedparser.parse(url)):
                ...
                if download_link:
                    ...
                    if self.db.retrieve(key) == 'added' or self.db.retrieve(key) == 'notdl' or self.db.retrieve(key.replace(".COMPLETE", "").replace(".Complete", "")) == 'added':
                        ...
                        common.write_crawljob_file( key, key, download_link, jdownloaderpath + "/folderwatch", "RSScrawler")
                        self.db.store( key, 'notdl' if self.config.get('enforcedl') and '.dl.' not in key.lower() else 'added')
                        log_entry = '[Film] - ' + ('<b>Englisch</b> - ' if englisch and not retail else "") + ('<b>Englisch/Retail</b> - ' if englisch and retail else "") + ('<b>Retail</b> - ' if not englisch and retail else "") + key + ' - <a href="' + download_link + key + '&#39;)" title="Download f&uuml;r n&auml;chsten Suchlauf zur&uuml;cksetzen"><i class="fas fa-undo"></i></a>'
                        self.log_info(log_entry)
                        added_items.append(log_entry)
                    elif self.filename == 'MB_3D':
                        retail = False
                ...
                if found:
                    ...
                    if self.filename == 'MB_Filme':
                        ...
                        if '.3d.' in title.lower():
                            if self.config.get('crawl3d') and ("1080p" in title.lower() or "1080i" in title.lower()):
                                if not self.config.get("crawl3dtype"):
                                    c3d_type = "hsbs"
                                else:
                                    c3d_type = self.config.get("crawl3dtype")
                                if c3d_type == "hsbs":
                                    if re.match(r'.*\.(H-OU|HOU)\..*', title):
                                        self.log_debug("%s - Release ignoriert (Falsches 3D-Format)" % title)
                                        continue
                                elif c3d_type == "hou":
        ...
        for link in dl_links:
            ...
            if self._hosters_pattern.lower().replace(" ", "-") in link[1].lower().replace(" ", "-"):
                ...
                if found:
                    found = re.search(ignore, title.lower())
                    if found:
                        self.log_debug("%s - zweisprachiges Release ignoriert (basierend auf ignore-Einstellung)" % title)
                        continue
            ...
            if self.config.get("historical"):
                for xline in list(self.allInfos.keys()):
                    if len(xline) > 0 and not xline.startswith("#"):
                        ...
                        urls.append(str(base64.b64decode("aHR0cDovL3d3dy5oZC1hcmVhLm9yZy8/cz1zZWFyY2gmcT0="), 'utf-8') + search_title)
            else:
                urls.append(self.FEED_URL)
        else:
            urls.append(self.FEED_URL)
