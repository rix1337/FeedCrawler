...
function getLogPages() {
	if (typeof log.value !== "undefined") {
		...
		if (resLengthLog.value > 0) {
			numberOfPagesLog.value = Math.ceil(
				resLengthLog.value / pageSizeLog.value
			);
		} else {
...
const currentLogPage = computed(() => {
	...
	return log.value.slice(
		(currentPageLog.value - 1) * pageSizeLog.value,
		currentPageLog.value * pageSizeLog.value
	);
});
...
function deleteLogRow(title) {
	...
	axios.delete("api/log_entry/" + title_b64).then(
		function () {
		...
		<div id="log" class="col">
			...
			<div class="card log">
				<table v-if="log.length > 0" class="table">
					<thead>
						<tr>
							<th class="text-left d-none d-lg-block" scope="col">
								Zeitstempel
							</th>
						...
						<tr v-for="x in currentLogPage">
							...
							<td class="text-left">
								...
								<button v-if="!longLogItemsAllowed && checkEntryLength(x[3])" class="btn btn-link btn-sm"
									v-tippy="'Titel vollständig anzeigen'" @click="longerLog()">
									...
								</button>
								<button v-if="longLogItemsAllowed && checkEntryLength(x[3])" v-tippy="'Titel kürzen'"
									class="btn btn-link btn-sm" @click="shorterLog()">
							...
							<td class="text-right">
								<button v-tippy="'Log-Eintrag löschen'" class="btn btn-link btn-sm" @click="deleteLogRow(x[3])">
			...
			<div v-if="resLengthLog > 5" class="btn-group">
				<paginate v-model="currentPageLog" :next-text="'>'" :page-count="numberOfPagesLog" :prev-text="'<'">
				</paginate>
			</div>
			<div>
