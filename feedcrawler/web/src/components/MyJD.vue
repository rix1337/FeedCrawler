	...
	axios.get("api/myjd/").then(
		function (res) {
			...
			if (myjd_failed.value) {
				...
				const failed_packages = Object.entries(
					res.data.packages.linkgrabber_failed
				);
...
function getMyJDPages() {
	if (typeof myjd_packages.value !== "undefined") {
		...
		if (resLengthMyJD.value > 0) {
			numberOfPagesMyJD.value = Math.ceil(
				resLengthMyJD.value / pageSizeMyJD.value
			);
		} else {
...
const currentMyJDPage = computed(() => {
	...
	return myjd_packages.value.slice(
		(currentPageMyJD.value - 1) * pageSizeMyJD.value,
		currentPageMyJD.value * pageSizeMyJD.value
	);
});
...
function myJDstart() {
	...
	axios.post("api/myjd_start/").then(
		function () {
			getMyJDstate();
...
function myJDpause(pause) {
	...
	axios.post("api/myjd_pause/" + pause).then(
		function () {
			getMyJDstate();
			if (pause) {
...
function myJDmove(linkids, uuid, name) {
	axios.post("api/myjd_move/" + linkids + "&" + uuid).then(
		function () {
...
function myJDremove(linkids, uuid, name) {
	axios.post("api/myjd_remove/" + linkids + "&" + uuid).then(
		function () {
			toast.success("Download\n" + name + "\ngelöscht!");
			if (myjd_failed.value) {
...
function myJDreset(linkids, uuid, name) {
	axios.post("api/myjd_reset/" + linkids + "&" + uuid).then(
		function () {
			toast.success("Paket\n" + name + "\nzurückgesetzt!");
			if (myjd_failed.value) {
				for (let failed_package of myjd_failed.value) {
...
function internalRemove(name) {
	axios.post("api/internal_remove/" + name).then(
		function () {
			toast.success("Download\n" + name + "\ngelöscht!");
			if (to_decrypt.value) {
				for (let failed_package of to_decrypt.value) {
...
function myJDretry(linkids, uuid, links, name) {
	links = btoa(links);
	axios.post("api/myjd_retry/" + linkids + "&" + uuid + "&" + links).then(
		function () {
			toast.success("Download\n" + name + "\nerneut hinzugefügt!");
			if (myjd_failed.value) {
				for (let failed_package of myjd_failed.value) {
...
function internalCnl(name, password) {
	...
	countDown();
	axios
		.post("api/internal_cnl/" + name + "&" + password)
		.then(function () {
...
function showSponsorsHelp() {
	let offcanvas = new Offcanvas(
		document.getElementById("offcanvasBottomHelp"),
		{ backdrop: false }
	);
	offcanvas.show();
				...
				<div class="accordion-item myjdheader">
					<h2 id="headingMyJd" class="accordion-header">
						<button id="myjd_collapse" aria-controls="collapseMyJd" aria-expanded="false"
							class="accordion-button collapsed" data-bs-target="#collapseMyJd" data-bs-toggle="collapse" type="button"
							@click="manualCollapse">
							Details
						</button>
					</h2>
									...
									<div v-if="x.type == 'online'" class="card bg-success">
										...
										<ul class="list-group list-group-flush">
											<li class="list-group-item">
												<span v-text="x.done"></span> /
												<span v-text="x.size"></span>
											</li>
											<li class="list-group-item">
												<div class="progress">
													<div :class="{
														'bg-info': x.eta_ext,
														'bg-warning': !x.speed && !x.eta_ext,
													}" :style="{ width: x.percentage + '%' }" aria-valuemax="100" aria-valuemin="0"
														aria-valuenow="x.percentage" class="
															progress-bar
															progress-bar-striped
															progress-bar-animated
														" role="progressbar">
														<div id="percentage">
															<span v-text="x.percentage"></span> %
														</div>
													</div>
												</div>
											</li>
										...
										<ul class="list-group list-group-flush">
											<li class="list-group-item">
												<button class="btn btn-outline-danger" @click="myJDremove(x.linkids, x.uuid, x.name)">
									...
									<div v-if="x.type == 'decrypted'" class="card bg-warning">
										...
										<ul class="list-group list-group-flush">
											<li v-if="cnl_active" class="list-group-item">
												<span v-tippy="'Warte auf hinzugefügte Links!'" class="cnl-spinner">
													<span class="spinner-border spinner-border-sm" role="status"></span>
													Warte auf hinzugefügte Links!</span>
											</li>
											<li v-if="x.size" class="list-group-item">
												<span v-text="x.size"></span>
											</li>
											<li v-if="!cnl_active" class="list-group-item cnl-blockers">
												<button v-tippy="'Download starten'" class="btn btn-outline-success"
													@click="myJDmove(x.linkids, x.uuid, x.name)">
								...
								<div class="myjd-failed">
									<div v-if="x.type == 'failed'" class="card bg-danger">
										...
										<ul class="list-group list-group-flush">
											<li class="list-group-item">
												<span v-tippy="
													'Dies tritt auf, wenn das Entpacken fehlschlägt, oder Teile des Paketes offline sind.'
												">
											...
											<li class="list-group-item">
												<button v-if="!cnl_active" class="btn btn-outline-danger"
													@click="myJDreset(x.linkids, x.uuid, x.name)">
													<i class="bi bi-arrow-clockwise"></i>
													Zurücksetzen
												</button>
												<button v-if="!cnl_active" class="btn btn-outline-danger"
													@click="myJDremove(x.linkids, x.uuid, x.name)">
													<i class="bi bi-trash"></i>
													Löschen
												</button>
											</li>
									...
									<div v-if="x.type == 'to_decrypt'" class="card bg-danger">
										...
										<ul class="list-group list-group-flush">
											<li v-if="x[1].url" class="list-group-item">
												<a v-if="
													store.state.misc.helper_active &&
													store.state.misc.helper_available &&
													x[1].first &&
													!cnl_active
												" :href="x[1].url + '#' + x[1].name" class="cnl-button btn btn-outline-success" target="_blank" v-tippy="
	'Da der Click\'n\'Load des FeedCrawler Sponsors Helper verfügbar ist, kann die Click\'n\'Load Automatik hiermit umgangen werden.'
" type="submit">Sponsors Helper Click'n'Load
												...
												">Bitte zuerst
													...
													<a :href="
														context +
														'./sponsors_helper/feedcrawler_sponsors_helper_sj.user.js'
													" target="_blank">FeedCrawler Sponsors Helper (SJ)</a>
													installieren!
												</span>
												<span v-if="
												...
												">Bitte zuerst
													...
													<a :href="
														context +
														'./sponsors_helper/feedcrawler_sponsors_helper_sj.user.js'
													" target="_blank">FeedCrawler Sponsors Helper (DJ)</a>
													installieren!
												</span>
												<span v-if="
													(x[1].url.includes('filecrypt') ||
												...
												">Bitte zuerst
													...
													<a :href="
														store.state.prefix +
														context +
														'./sponsors_helper/feedcrawler_sponsors_helper_fc.user.js'
													" target="_blank">FeedCrawler Sponsors Helper (FC)</a>
													installieren!
												</span>
												<a v-if="!myjd_grabbing && !cnl_active" :href="x[1].url + '#' + x[1].name"
													class="cnl-button btn btn-secondary" v-tippy="
														'Click\'n\'Load innerhalb einer Minute auslösen!'
													" target="_blank" type="submit" @click="internalCnl(x[1].name, x[1].password)">Click'n'Load-Automatik
												</a>
												<span v-if="!myjd_grabbing">Setzt voraus, dass Port 9666 des JDownloaders durch
													diese Browsersitzung erreichbar ist.</span>
												<span v-if="
													store.state.hostnames.sj &&
													x[1].url.includes(
														store.state.hostnames.sj.toLowerCase()
													)
												"><br />Bitte zuerst
													<a href="https://www.tampermonkey.net/" target="_blank">Tampermonkey</a>
													und dann
													<a :href="
														context +
														'./sponsors_helper/feedcrawler_helper_sj.user.js'
													" target="_blank">FeedCrawler Helper (SJ)</a>
													installieren!
												</span>
												<span v-if="
													store.state.hostnames.dj &&
													x[1].url.includes(
														store.state.hostnames.dj.toLowerCase()
													)
												"><br />Bitte zuerst
													<a href="https://www.tampermonkey.net/" target="_blank">Tampermonkey</a>
													und dann
													<a :href="
														context +
														'./sponsors_helper/feedcrawler_helper_sj.user.js'
													" target="_blank">FeedCrawler Helper (DJ)</a>
													installieren!
												</span>
												<span v-if="!store.state.misc.helper_active"><br />
													<mark>Genervt davon, CAPTCHAs manuell zu lösen? Jetzt
														<a v-tippy="
															'Bitte unterstütze die Weiterentwicklung über eine aktive Github Sponsorship!'
														" target="_blank" href="https://github.com/users/rix1337/sponsorship">Sponsor werden</a>
														und den
														<a href="#" @click="showSponsorsHelp()">den Sponsors Helper</a>
														für dich arbeiten lassen.</mark>
												</span>
												<span v-if="myjd_grabbing"><br />Die Click'n'Load-Automatik funktioniert nicht
													bei aktivem Linkgrabber.</span>
												<span v-if="cnl_active" class="cnl-spinner"><br />
													<span class="spinner-border spinner-border-sm" role="status"></span>
													<strong>Warte noch {{ time }}
														{{ time == 1 ? "Sekunde" : "Sekunden" }} auf
														hinzugefügte Links!</strong>
												</span>
											</li>
											<li v-if="!cnl_active" class="list-group-item cnl-blockers">
												<button v-if="!cnl_active" class="btn btn-outline-danger" @click="internalRemove(x[1].name)">
													<i class="bi bi-trash"></i>
													Löschen
												</button>
											</li>
								...
								<div class="myjd-offline">
									<div v-if="x.type == 'offline'" class="card bg-danger">
										...
										<ul class="list-group list-group-flush">
											<li class="list-group-item">
												<button v-if="!cnl_active" v-tippy="'Erneut hinzufügen'" class="btn btn-outline-info"
													@click="myJDretry(x.linkids, x.uuid, x.urls, x.name)">
							...
							<div v-if="resLengthMyJD > 3" class="btn-group">
								<paginate v-model="currentPageMyJD" :next-text="'>'" :page-count="numberOfPagesMyJD" :prev-text="'<'">
								</paginate>
							</div>
							...
							<div v-if="myjd_state && myjd_packages.length == 0" id="myjd_no_packages" class="myjd_connection_state">
								Downloadliste und Linksammler sind leer.
							</div>

							<div v-if="myjd_downloads" id="myjd_state">
								<span v-if="
									myjd_state == 'STOPPED_STATE' || myjd_state == 'STOPPING'
								" id="myjd_start" :class="{
	blinking: myjd_starting,
	isDisabled: myjd_starting,
}" @click="myJDstart()">
								...
								<span v-if="myjd_state == 'RUNNING' || myjd_state == 'PAUSE'" id="myjd_stop" :class="{
									blinking: myjd_stopping,
									isDisabled: myjd_stopping,
								}" @click="myJDstop()"><i v-tippy="'Downloads anhalten'" class="bi bi-stop"></i></span>
							</div>
						</div>
					</div>
				</div>
