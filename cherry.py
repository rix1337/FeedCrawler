...
<html lang="de">
  <head>
    ...
    <script>
      $(document).ready(function(){$("#smbr").on("change",function(){"True"==this.value?$("#dmbr").show():$("#dmbr").hide()}),$("#smb3").on("change",function(){"True"==this.value?$("#dmb3").show():$("#dmb3").hide()}),$("#smbs").on("change",function(){"True"==this.value?($("#dmbss").show(),$("#dmbs").show(),$("#hs").show()):($("#dmbss").hide(),$("#dmbs").hide(),$("#hs").hide())}),$("#ssjr").on("change",function(){"True"==this.value?$("#dsjr").show():$("#dsjr").hide()}),$("#ytopt").on("change",function(){"True"==this.value?($("#dyt").show(),$("#sdyt").show(),$("#hy").show()):($("#dyt").hide(),$("#sdyt").hide(),$("#hy").hide())})});
    </script>
    </script>
    <script>
  ...
  <div class="container">
    ...
    <form id="rsscrawler" name="log" enctype="multipart/form-data" target="speichernFrame" method="post" action="logleeren?wert=1">
          <h1>Log</h1>
            <iframe sandbox="allow-forms allow-same-origin allow-pointer-lock allow-scripts allow-popups allow-modals" src="./log" width="800" height="400">
    ...
    <form id="rsscrawler" enctype="multipart/form-data" target="speichernFrame" method="post" action="listenspeichern">
          <h1>Suchlisten</h1>
          <div hinweis="Dieser Bereich ist für die Suche auf allgemeinen Blogs zuständig."><h3>Allgemeine Blogs</h3></div>
          ...
          <div style='display:''' + ssdiv +''';' id='hs' hinweis="Dieser Bereich ist für die Suche auf ''' + "YWxsZ2VtZWluZW4gQmxvZ3M=".decode('base64') +  ''' und ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + ''' zuständig."><h3>''' + "QWxsZ2VtZWluZSBCbG9ncw==".decode('base64') +  ''' und ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + '''</h3></div>
    ...
    <form id="rsscrawler" enctype="multipart/form-data" target="speichernFrame" method="post" action="speichern">
          ...
          Suchfunktion, statt Feed nutzen:<div hinweis="Wenn aktiviert, wird die Suchfunktion des Allgemeine Blogs genutzt, da der Feed nur wenige Stunden abbildet."><select name="historical"><option value="True"''' + historicaltrue + '''>Aktiviert</option><option value="False"''' + historicalfalse + '''>Deaktiviert</option></select></div>
          ...
          <div hinweis="Dieser Bereich ist für die Suche auf allgemeinen Blogs und ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + '''  zuständig."><h3>Allgemeine Blogs und ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + '''</h3></div>
          Staffeln suchen:<div hinweis="Wenn aktiviert, werden komplette Staffeln entsprechend der Staffel-Liste auf Allgemeine Blogs und ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + ''' gesucht."><select id="smbs" name="crawlseasons"><option value="True"''' + crawlseasonstrue + '''>Aktiviert</option><option value="False"''' + crawlseasonsfalse + '''>Deaktiviert</option></select></div>
          <div style='display:''' + ssdiv +''';' id='dmbss'>Auflösung der Staffeln:<div hinweis="Die Release-Auflösung der Staffeln, nach der gesucht wird."><select name="seasonsquality"><option value="2160p"''' + msq2160 + '''>4K</option><option value="1080p"''' + msq1080 + '''>Full-HD</option><option value="720p"''' + msq720 + '''>HD</option><option value="480p"''' + msq480 + '''>SD</option></select></div>
          Staffelpakete hinzufügen:<div hinweis="Wenn aktiviert, werden auch Staffelpakete hinzugefügt, die häufig mehrere hundert Gigabyte groß sind."><select name="seasonpacks"><option value="True"''' + spacktrue + '''>Aktiviert</option><option value="False"''' + spackfalse + '''>Deaktiviert</option></select></div>
          Quellart der Staffeln:<div hinweis="Die Quellart der Staffeln, nach der gesucht wird."><select name="seasonssource"><option value="hdtv|hdtvrip|tvrip"''' + shdtv + '''>HDTV</option><option value="web-dl|webrip|webhd|netflix*|amazon*|itunes*"''' + sweb + '''>WEB</option><option value="hdtv|hdtvrip|tvrip|web-dl|webrip|webhd|netflix*|amazon*|itunes*"''' + shdtvweb + '''>HDTV & WEB</option><option value="bluray|bd|bdrip"''' + sbluray + '''>BluRay</option><option value="web-dl|webrip|webhd|netflix*|amazon*|itunes*|bluray|bd|bdrip"''' + swebbluray + '''>BluRay & Web</option><option value="hdtv|hdtvrip|tvrip|web-dl|webrip|webhd|netflix*|amazon*|itunes*|bluray|bd|bdrip"''' + shdtvwebbluray + '''>HDTV & WEB & BluRay</option><option value="web-dl.*-(tvs|4sj)|webrip.*-(tvs|4sj)|webhd.*-(tvs|4sj)|netflix.*-(tvs|4sj)|amazon.*-(tvs|4sj)|itunes.*-(tvs|4sj)|bluray|bd|bdrip"''' + swebretailbluray + '''>BluRay & WebRetail (TVS/4SJ)</option></select></div></div>
...
<html lang="de">
  ...
  <div class="container">
    ...
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="..''' + prefix +'''/logleeren?wert=1">
          <h1>Log</h1>
            <iframe sandbox="allow-forms allow-same-origin allow-pointer-lock allow-scripts allow-popups allow-modals" src="./..''' + prefix +'''/log" width="800" height="400">
    ...
    <form id="rsscrawler" action="https://www.9kw.eu/register_87296.html" target="_blank">
          <h1>Hilfe</h1>
          <div id="regex-hinweise"><h3>Beispiele für allgemeine Blogs</h3><i>Film.*.Titel.*</i><br />Sucht nach allen Filmen die mit Film beginnen und Titel enthalten.<br /><i>.*-GROUP</i> oder <i>-GROUP</i><br />sucht nach allen Releases der GROUP.<br /><i>.*1080p.*-GROUP</i><br />sucht nach allen Full-HD Releases der GROUP.<br /><i>Film.Titel.*.DL.*.720p.*.BluRay.*-Group</i><br/>sucht nach HD-BluRay Releases von Film Titel, zweisprachig und in 720p der GROUP.<h3>Beispiele für ''' + "U2VyaWVuSnVua2llcw==".decode('base64') + '''</h3><i>DEUTSCH.*Serien.Titel.*.S01.*.720p.*-GROUP</i><br />sucht nach Releases der GROUP von Staffel 1 der Serien Titel in 720p auf Deutsch.<br /><i>Serien.Titel.*</i><br />sucht nach allen Releases von Serien Titel (nützlich, wenn man sonst HDTV aussortiert).<br /><i>Serien.Titel.*.DL.*.720p.*</i><br />sucht nach zweisprachigen Releases in 720p von Serien Titel.<br /><i>ENGLISCH.*Serien.Titel.*.1080p.*</i><br />sucht nach englischen Releases in Full-HD von Serien Titel.<br /><i>(?!(Diese|Andere)).*Serie.*.DL.*.720p.*-(GROUP|ANDEREGROUP)</i><br />sucht nach Serie (aber nicht Diese Serie oder Andere Serie), zweisprachig und in 720p und ausschließlich nach Releases von GROUP oder ANDEREGROUP.<h3>All diese Regeln lassen sich beliebig kombinieren.</h3>Falsche Regex-Einträge können allerdings das Script zum Absturz bringen!</div>
          <button type="submit">Captchas automatisch lösen</button>
    </form>
