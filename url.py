...
def checkURL():
    ...
    if proxy:
        ...
        if "block." in str(scraper.get(sj_url, proxies=proxies, timeout=15, allow_redirects=False).headers.get("location")):
            ...
            db.delete("SJ")
        if "<Response [403]>" in str(scraper.get(mb_url, proxies=proxies, timeout=15, allow_redirects=False)):
    ...
    if not proxy or sj_blocked_proxy == True or mb_blocked_proxy == True:
        if "block." in str(scraper.get(sj_url, timeout=15, allow_redirects=False).headers.get("location")):
            print "Der Zugriff auf SJ ist mit der aktuellen IP nicht möglich!"
        if "<Response [403]>" in str(scraper.get(mb_url, timeout=15, allow_redirects=False)):
    ...
    if proxy:
        ...
        if sj in url:
            if db.retrieve("SJ") and config.get("fallback"):
                return scraper.get(url, timeout=15).content
        elif mb in url:
            if db.retrieve("MB") and config.get("fallback"):
                return scraper.get(url, timeout=15).content
        proxies = {'http': proxy, 'https': proxy}
        return scraper.get(url, proxies=proxies, timeout=15).content
    else:
        return scraper.get(url, timeout=15).content
...
def postURL(url, data):
    ...
    if proxy:
        proxies = {'http': proxy, 'https': proxy}
        scraper = cfscrape.create_scraper(delay=10)
        return scraper.post(url, data, proxies=proxies, timeout=15).content
    else:
        scraper = cfscrape.create_scraper(delay=10)
        return scraper.post(url, data, timeout=15).content
